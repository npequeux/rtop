...
# Test 1: Binary size
echo "1. Binary Size:"
ls -lh target/release/mytop | awk '{print "   "$5}'

# Test 2: Startup time
echo ""
echo "2. Startup Time (5 runs):"
for i in {1..5}; do
    /usr/bin/time -f "   Run $i: %E elapsed, %M KB max memory" timeout 1 ./target/release/mytop 2>&1 | grep "Run"
done

# Test 3: Memory usage
echo ""
echo "3. Memory Usage Test (2 seconds):"
./target/release/mytop &
PID=$!
sleep 0.5
...
PID=$!
sleep 1
top -b -n 3 -d 1 -p $PID 2>/dev/null | grep mytop | awk '{sum+=$9; count++} END {if(count>0) print "   Average CPU: " sum/count "%"}'
kill $PID 2>/dev/null
wait $PID 2>/dev/null
